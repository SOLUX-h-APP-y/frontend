...
const ChatScreen = ({ route, navigation }) => {
  ...
  const [messages, setMessages] = useState([
    {
      ...
      sender_id: 2,
      content:
        '안녕하세요. 카메라 한달동안 빌리고 싶은데 하루 5000원으로 가능할까요? ㅎㅎ',
      is_read: 1,
  ...
  const renderItem = ({ item, index }) => {
    const currentDate = new Date(item.create_at)
    ...
    const prevDate =
      index > 0
        ? new Date(messages[index - 1].create_at)
            .toLocaleDateString('ko-KR', {
              year: 'numeric',
              month: '2-digit',
              day: '2-digit',
        ...
        {isCompleted && (
          <View style={styles.reviewContainer}>
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate('ReviewScreen', { chatRoomId })
              }>
